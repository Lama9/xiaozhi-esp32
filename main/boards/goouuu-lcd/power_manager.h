...
private:
    ...
    void ReadBatteryAdcData() {
        ...
        const struct {
            ...
            {580, 0},   // 3.0V电池电压 → 0.479V ADC电压 → 580 ADC值
            {620, 20},  // 3.3V电池电压 → 0.526V ADC电压 → 620 ADC值
            {670, 40},  // 3.6V电池电压 → 0.574V ADC电压 → 670 ADC值
            {720, 60},  // 3.8V电池电压 → 0.606V ADC电压 → 720 ADC值
            {770, 80},  // 4.0V电池电压 → 0.638V ADC电压 → 770 ADC值
            {830, 100}  // 4.174V电池电压 → 0.666V ADC电压 → 830 ADC值 (最新实测数据)
        };
        ...
        // 计算实际电池电压（根据分压器计算）
        // 最新实测分压比 = 0.1595 (根据实测数据：ADC电压0.666V / 电池电压4.174V)
        // 理论分压比 = 1K / (4.7K + 1K) = 0.175
        // 实际分压比与理论值有差异，使用最新实测值进行校准
        // 电压转换公式：
        // ADC电压 = ADC值 * 3.3V / 4095
        // 电池电压 = ADC电压 / 实际分压比
        float adc_voltage = (float)average_adc * 3.3f / 4095.0f;
        float battery_voltage = adc_voltage / 0.1595f; // 使用最新实测分压比进行校准
...
public:
    ...
    float GetBatteryVoltage() {
        ...
        // 根据最新实测分压比计算实际电池电压
        // 最新实测分压比 = 0.1595 (根据实测数据校准)
        return adc_voltage / 0.1595f;
    }
