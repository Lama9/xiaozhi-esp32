...
def get_idf_cmd() -> str:
    ...
    if not idf_path:
        print("[ERROR] IDF_PATH 环境变量未设置，请先运行 export.bat 或 get_idf", file=sys.stderr)
        sys.exit(1)
    
    idf_script = Path(idf_path) / "tools" / "idf.py"
    return f'"{sys.executable}" "{idf_script}"'
...
def merge_bin() -> None:
    if os.system(f'"{IDF_CMD} merge-bin"') != 0:
...
def release(board_type: str, config_filename: str = "config.json", *, filter_name: Optional[str] = None) -> None:
    ...
    for build in builds:
        ...
        # Call set-target
        if os.system(f'"{IDF_CMD} set-target {target}"') != 0:
        ...
        if os.system(f'"{IDF_CMD} -DBOARD_NAME={name} -DBOARD_TYPE={board_type} build"') != 0:
            print("build failed")
            sys.exit(1)
